...
| 1.3.0 | **applied** — in two parts | (a) The `fan-out` -> `orchestrate` rename this release performed: the refreshed `docs/tiers.md` XL row reads `mandatory orchestrate`, carried by the 1.4.0 template refresh. The `AGENTS.md` tier one-liner was updated to `XL orchestrate` in the same step for consistency with it (rewrapped to keep the phrase on one line) — note 1.3.0 does not itself instruct editing that line, which 0.6.0 installed; the edit follows from the template, not from this note. (b) "the workstation installer must sweep the dangling fan-out junction" — **applied per Ruling A above**, as a dangling-only sweep plus a guard test, carrying `Closes MAT-50`. Implemented by **PLAN step 5**, not by this step; `claude/install.ps1` and `tests/run.ps1` were deliberately left untouched here. |
